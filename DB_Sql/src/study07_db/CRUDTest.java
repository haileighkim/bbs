...
public class CRUDTest {
	...
	public void insert() {
		...
		try {
			...
			String query = "insert into member1 values('aaa','1111','Hong','22','Seoul','a@a.com')";
			int count = stmt.executeUpdate(query);
			if (count > 0)
				System.out.println("Data가 저장되었습니다.");
			else
				System.out.println("Data가 저장되지 않았습니다.");
	...
	public void select() {
		...
		try {
			...
			System.out.println("No Driver : Class not Found");
	...
	public void update() {
		...
		try {
			...
			conn = DriverManager.getConnection(url, "javalink", "javalink");
			stmt = conn.createStatement();
			String query = "update member1 set addr ='Busan' where id = 'aaa'";
			int count = stmt.executeUpdate(query);
			if (count > 0)
...
//id=aaa 삭제
		...
		try {
			...
			int count = stmt.executeUpdate(query);
			if (count > 0)
				System.out.println("Data가 삭제되었습니다.");
			else
				System.out.println("Data가 저장되지 않았습니다.");
